    ...
    <div class="relative p-4 w-full max-w-2xl max-h-full">
        <div class="relative bg-white rounded-3xl shadow-xl border border-zinc-200">
            <div class="flex items-center justify-between px-5 py-4 border-b border-zinc-100">
                ...
                <button type="button" class="text-zinc-400 hover:bg-zinc-100 hover:text-zinc-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center transition-colors focus:outline-none" data-modal-hide="add-modal">
            ...
            <div class="p-5 space-y-5 max-h-[70vh] overflow-y-auto hide-scrollbar">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" name="name" value="{{ old('name') }}" required class="bg-zinc-50 border border-zinc-200 text-zinc-800 text-xs font-medium rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 block w-full p-2.5 transition-colors" placeholder="Masukkan nama..." />
                    ...
                    <div>
                        ...
                        <input type="email" name="email" value="{{ old('email') }}" required class="bg-zinc-50 border border-zinc-200 text-zinc-800 text-xs font-medium rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 block w-full p-2.5 transition-colors" placeholder="email@contoh.com" />
                    ...
                    <div>
                        ...
                        <input type="text" name="phone_number" value="{{ old('phone_number') }}" class="bg-zinc-50 border border-zinc-200 text-zinc-800 text-xs font-medium rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 block w-full p-2.5 transition-colors" placeholder="08..." />
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                        @foreach(\App\Models\Role::all() ?? [] as $role)
                            <label class="relative flex items-center justify-center p-3 border border-zinc-200 rounded-xl cursor-pointer hover:bg-zinc-50 transition-colors" :class="{ 'bg-emerald-50/50 border-emerald-200 ring-1 ring-emerald-500/20': roles.includes('{{ $role->slug }}'), 'opacity-60 cursor-not-allowed bg-zinc-50': ('{{ $role->slug }}' === 'lecturer' && roles.includes('admin_lab')) || ('{{ $role->slug }}' === 'student' && roles.includes('assistant')) || (['student', 'assistant'].includes('{{ $role->slug }}') && hasLecturer()) || (['lecturer', 'admin_lab'].includes('{{ $role->slug }}') && hasStudent()) }">
                                ...
                                <span class="text-xs font-bold text-zinc-700 select-none" :class="{ 'text-emerald-700': roles.includes('{{ $role->slug }}') }">{{ $role->name }}</span>
                            </label>
                        @endforeach
                    </div>
                ...
                <div x-show="hasStudent()" x-cloak class="space-y-4 p-4 rounded-xl bg-zinc-50/50 border border-zinc-100">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="nim" value="{{ old('nim') }}" :required="hasStudent()" class="bg-white border border-zinc-200 text-zinc-800 text-xs font-medium rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 block w-full p-2.5 transition-colors" placeholder="Masukkan NIM..." />
                        ...
                        <div>
                            ...
                            <input type="number" name="entry_year" value="{{ old('entry_year') }}" :required="hasStudent()" class="bg-white border border-zinc-200 text-zinc-800 text-xs font-medium rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 block w-full p-2.5 transition-colors" placeholder="2024" />
                ...
                <div x-show="hasLecturer()" x-cloak class="space-y-4 p-4 rounded-xl bg-zinc-50/50 border border-zinc-100">
                    ...
                    <div>
                        ...
                        <input type="text" name="nip" value="{{ old('nip') }}" :required="hasLecturer()" class="bg-white border border-zinc-200 text-zinc-800 text-xs font-medium rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 block w-full p-2.5 transition-colors" placeholder="Masukkan NIP..." />
                ...
                <div x-show="roles.length > 0" x-cloak>
                    ...
                    <select name="department_id" class="bg-zinc-50 border border-zinc-200 text-zinc-800 text-xs font-semibold rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 block w-full p-2.5 transition-colors">
                ...
                <div class="p-3 bg-emerald-50/50 border border-emerald-100 rounded-xl flex items-start gap-3">
                    <x-atoms.icon name="information-circle" class="w-5 h-5 text-emerald-500 shrink-0 mt-0.5" />
                    <div>
                        <p class="text-xs font-bold text-emerald-900">Password Default</p>
                        <p class="text-[11px] font-medium text-emerald-700/80 mt-0.5">Password pengguna otomatis diatur ke <b>LabUIN@2026</b> dan pengguna wajib mengganti password saat login pertama.</p>
                    </div>
                </div>
            </div>
            <div class="flex items-center justify-end px-5 py-4 border-t border-zinc-100 bg-zinc-50/50 rounded-b-3xl gap-3">
                <button data-modal-hide="add-modal" type="button" class="py-2.5 px-4 text-xs font-semibold text-zinc-700 bg-white border border-zinc-200 hover:bg-zinc-50 rounded-xl transition-colors">Batal</button>
                <button type="submit" class="text-white bg-emerald-600 hover:bg-emerald-700 font-semibold rounded-xl text-xs px-4 py-2.5 transition-colors shadow-sm focus:outline-none focus:ring-2 focus:ring-emerald-500">Tambah Pengguna</button>
            </div>
        </div>
    </div>
</form>
