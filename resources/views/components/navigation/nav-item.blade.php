...
@php
    ...
    // Base classes
    $baseClasses = 'group flex items-center transition-all duration-200 outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 relative';
    ...
    if ($isActive) {
        $stateClasses = $nested 
            ? 'bg-indigo-50/80 text-zinc-900 font-semibold shadow-sm ring-1 ring-black/5' 
            : 'bg-indigo-50 text-zinc-900 font-semibold shadow-sm ring-1 ring-black/5';
...
<li class="relative">
    @if($isActive && !$nested)
        ...
        <div 
            class="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-5 bg-indigo-600 rounded-r-full z-10 pointer-events-none"
            @if(!$isMobile)
