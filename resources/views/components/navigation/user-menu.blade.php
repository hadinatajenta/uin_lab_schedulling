    ...
    <x-ui.dropdown align="top-right" width="auto">
        ...
        <x-slot name="content">
            <x-ui.dropdown-header title="My Account">
                <p class="text-[13px] font-medium text-zinc-900 truncate">{{ Auth::user()->name ?? 'User Name' }}</p>
                <p class="text-[11px] text-zinc-500 truncate">{{ Auth::user()->email ?? 'user@example.com' }}</p>
