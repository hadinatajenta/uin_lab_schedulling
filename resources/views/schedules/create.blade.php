...
@section('content')
    <div class="px-4 py-8 mx-auto max-w-7xl sm:px-6 lg:px-8">
        ...
        <form action="{{ route('addJadwal') }}" method="POST" x-data="{
            ...
            }" @submit="isSubmitting = true" class="space-y-12">
            @csrf

            <!-- SECTION 1: Detail Praktikum -->
            <div class="grid grid-cols-1 gap-x-8 gap-y-10 md:grid-cols-3">
                <div>
                    <h2 class="text-base font-semibold leading-7 text-zinc-900">Detail Praktikum</h2>
                    <p class="mt-1 text-sm leading-6 text-zinc-500">Informasi spesifik mata kuliah, kelas, dan dosen
                        pengampu.</p>
                </div>
                ...
                <div class="grid max-w-2xl grid-cols-1 gap-x-6 gap-y-6 sm:grid-cols-6 md:col-span-2">
                    ...
                    <div class="col-span-full">
                        ...
                        <x-input-error :messages="$errors->get('submateri')" class="mt-2" />
                    </div>

                    <div class="col-span-full sm:col-span-4">
                        <x-input-label for="dosen_id" value="Dosen Pengampu" class="mb-1" />
                        <select id="dosen_id" name="dosen_id"
                            class="block w-full h-11 md:h-10 text-sm md:text-xs font-medium rounded-xl text-zinc-800 border-zinc-200 bg-white focus:outline-none focus:ring-2 focus:ring-[rgb(var(--color-primary)_/_0.2)] focus:border-[rgb(var(--color-primary))] shadow-sm transition-colors"
                            required>
                    ...
                    <div class="col-span-3 sm:col-span-1">
                        ...
                        <x-ui.input id="semester" name="semester" type="number" :value="old('semester')" required min="1"
                            max="14" placeholder="Cth: 3" />
                        <x-input-error :messages="$errors->get('semester')" class="mt-2" />
                    </div>
                </div>
            </div>

            <div class="border-t border-zinc-900/5"></div>

            <!-- SECTION 2: Ruangan -->
            <div class="grid grid-cols-1 gap-x-8 gap-y-10 md:grid-cols-3">
                <div>
                    <h2 class="text-base font-semibold leading-7 text-zinc-900">Pemilihan Ruangan</h2>
                    <p class="mt-1 text-sm leading-6 text-zinc-500">Tentukan ruangan laboratorium yang akan digunakan untuk
                        praktikum ini.</p>
                </div>

                <div class="max-w-2xl md:col-span-2">
                    @error('ruangan_id')
                        <div class="mb-4 p-3 rounded-lg bg-rose-50 text-sm text-rose-600 font-medium">
                            {{ $message }}
                        </div>
                    @enderror
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        @forelse ($ruangans ?? [] as $ruang)
                            <label
                                class="relative flex flex-col p-4 border border-zinc-200 rounded-xl cursor-pointer hover:border-[rgb(var(--color-primary))] has-[:checked]:border-[rgb(var(--color-primary))] has-[:checked]:bg-[rgb(var(--color-primary)_/_0.03)] has-[:checked]:ring-1 has-[:checked]:ring-[rgb(var(--color-primary))] transition-all group">
                                <input type="radio" name="ruangan_id" value="{{ $ruang->id }}" class="sr-only" {{ old('ruangan_id') == $ruang->id ? 'checked' : '' }} required>
                                <div class="flex items-center justify-between mb-2">
                                    <span
                                        class="material-symbols-rounded text-zinc-400 group-has-[:checked]:text-[rgb(var(--color-primary))]">meeting_room</span>
                                    <div
                                        class="w-4 h-4 rounded-full border border-zinc-300 group-has-[:checked]:border-[rgb(var(--color-primary))] flex items-center justify-center">
                                        <div
                                            class="w-2 h-2 rounded-full bg-[rgb(var(--color-primary))] hidden group-has-[:checked]:block">
                                        </div>
                                    </div>
                                </div>
                                ...
                                <p class="text-xs text-zinc-500 mt-1">Kapasitas: {{ $ruang->kapasitas ?? '30' }} Orang</p>
                            </label>
                        @empty
                            <div class="col-span-full p-6 border border-dashed border-zinc-300 rounded-xl text-center">
                                <span class="material-symbols-rounded text-zinc-400 text-3xl">meeting_room</span>
                                <h4 class="text-sm font-medium text-zinc-900 mt-2">Ruangan Tidak Tersedia</h4>
            ...
            <div class="grid grid-cols-1 gap-x-8 gap-y-10 md:grid-cols-3">
                <div>
                    <h2 class="text-base font-semibold leading-7 text-zinc-900">Waktu Pelaksanaan</h2>
                    <p class="mt-1 text-sm leading-6 text-zinc-500">Tentukan tanggal dan jam (mulai hingga selesai).</p>
                </div>
                ...
                <div class="grid max-w-2xl grid-cols-1 gap-x-6 gap-y-6 sm:grid-cols-6 md:col-span-2">
                    <div class="col-span-full sm:col-span-3">
                        ...
                        <x-input-error :messages="$errors->get('tanggal_jadwal')" class="mt-2" />
                    </div>
                    ...
                    <div class="col-span-3 sm:col-span-2">
                        ...
                        <div x-show="!isTimeValid" x-transition class="mt-2 text-[11px] font-medium text-rose-600">
                            Waktu selesai harus lebih besar dari waktu mulai.
                        </div>
                        <x-input-error x-show="isTimeValid" :messages="$errors->get('waktu_selesai')" class="mt-2" />
                    </div>
                </div>
            </div>
            ...
            <div class="flex items-center justify-end gap-x-4 pt-4">
                <a href="{{ route('lab') }}"
                    class="text-sm font-semibold leading-6 text-zinc-900 hover:text-zinc-600">Batalkan</a>
                <x-primary-button type="submit" x-bind:disabled="!isTimeValid || isSubmitting">
