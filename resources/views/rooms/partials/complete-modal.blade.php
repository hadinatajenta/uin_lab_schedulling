...
<div x-data="{ show: false }" 
    ...
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div x-show="show" class="inline-block align-bottom bg-surface rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-3xl sm:w-full">
            ...
            <form action="{{ route('rooms.maintenance.complete', $maintenance->id) }}" method="POST">
                ...
                <div class="bg-surface px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <h3 class="text-lg leading-6 font-medium text-foreground mb-4 border-b pb-2">
                        Resolve: {{ $maintenance->title }}
                    </h3>
                    
                    <div class="space-y-6">
                        <div>
                            <label class="block text-sm font-medium text-foreground-muted">Notes / Laporan Penyelesaian</label>
                            <textarea name="notes" rows="3" class="mt-1 block w-full border-default rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" placeholder="Jelaskan tindakan yang telah dilakukan..."></textarea>
                        </div>
                        ...
                        @if($maintenance->is_emergency)
                            ...
                            <div class="bg-red-50 p-4 rounded-lg border border-red-200">
                                ...
                                @if($expiredSchedules->count() > 0)
                                    <div class="mb-6">
                                        <h5 class="text-sm font-medium text-foreground bg-default/50 px-3 py-1 rounded">Kelompok A: Jadwal Kadaluarsa ({{ $expiredSchedules->count() }})</h5>
                                        <p class="text-xs text-foreground-muted mt-1 mb-2">Jadwal ini tanggalnya sudah terlewat saat masa perbaikan.</p>
                                        <div class="space-y-3">
                                            @foreach($expiredSchedules as $i => $schedule)
                                                <div class="flex items-center justify-between text-sm border-b border-red-100 pb-2">
                                                    ...
                                                    <select name="schedule_recovery[expired_schedules][{{$i}}][action]" class="text-sm border-default rounded-md shadow-sm py-1 pl-2 pr-8 focus:border-indigo-500 focus:ring-indigo-500">
                                ...
                                @if($activeSchedules->count() > 0)
                                    <div>
                                        <h5 class="text-sm font-medium text-foreground bg-default/50 px-3 py-1 rounded">Kelompok B: Jadwal Mendatang ({{ $activeSchedules->count() }})</h5>
                                        <p class="text-xs text-foreground-muted mt-1 mb-2">Jadwal ini belum terjadi dan bisa dipulihkan atau dipindahkan.</p>
                                        <div class="space-y-3">
                                            @foreach($activeSchedules as $i => $schedule)
                                                <div class="flex flex-col sm:flex-row sm:items-center justify-between text-sm border-b border-red-100 pb-2 gap-2">
                                                    ...
                                                    <div class="flex gap-2">
                                                        <select name="schedule_recovery[active_schedules][{{$i}}][action]" class="text-sm border-default rounded-md shadow-sm py-1 pl-2 pr-8 focus:border-indigo-500 focus:ring-indigo-500">
                                                            <option value="resume">Aktifkan Kembali di Ruangan Ini</option>
                                                            <option value="transfer">Pindahkan ke Ruangan Lain</option>
                                                        </select>
                                                        <select name="schedule_recovery[active_schedules][{{$i}}][new_room_id]" class="text-sm border-default rounded-md shadow-sm py-1 pl-2 pr-8 focus:border-indigo-500 focus:ring-indigo-500">
                                ...
                                @if($suspendedSchedules->count() === 0)
                                    <p class="text-sm text-foreground-muted italic">Tidak ada jadwal yang terdampak selama masa darurat.</p>
                                @endif
                            </div>
                        @endif
                    </div>
                </div>
                <div class="bg-surface-muted px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse border-t">
